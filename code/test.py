...
class PatientData:
    ...
    def encoded_flag(self, patient_name):
        file_path = config.box_data_path
...
class PreprocessDataset:
    def __init__(
        ...
        self.box_data_path = box_data_path
    ...
    def get_kframes_and_annot_from_mhd(list_p):
        ...
        for patient in list_p:
            ...
            max_width = max([p.bounding_box[2] for p in list_p])
            max_height = max([p.bounding_box[3] for p in list_p])
        ...
        return lst_of_matrix_imgs, lst_of_annotation_imgs, lst_of_patient_names
...
class DeepValveDataset(Dataset):
    def __init__(self, patient_data_list, transform=None):
        ...
        (
            ...
            self.annotations,
            self.patient_names,
        ) = PreprocessDataset.get_kframes_and_annot_from_mhd(patient_data_list)
...
class DataSplitter:
    ...
    def split_data_balanced(self):
        ...
        print(
            f"The data has been split into {len(X_train)/68} for training, {len(X_val)/68} for validation, and {len(X_test)/68} for test."
        )

        return X_train, X_val, X_test


def get_data_loader(mode="train", normalize_keypts=True, num_workers=8):
    ...
    train_set = preprocess.get_patient_data_list(X_train, patient_data)
    val_set = preprocess.get_patient_data_list(X_val, patient_data)
    test_set = preprocess.get_patient_data_list(X_test, patient_data)

    train_dataset = DeepValveDataset(train_set)
    val_dataset = DeepValveDataset(val_set)
    test_dataset = DeepValveDataset(test_set)
    ...
    if mode == "train":
        ...
        return DataLoader(
            test_dataset, batch_size=4, shuffle=False, num_workers=num_workers
        )


# get_data_loader(mode = "train", normalize_keypts = True, num_workers=8)
