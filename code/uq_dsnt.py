...
def plot_heatmaps_with_spread(heatmaps, output_dir, case_id):
    ...
    spreads = [
        compute_spread(heatmaps[i].detach().cpu().numpy()) for i in range(num_heatmaps)
    ]
...
def run_and_plot_keypoints(
    model,
    dataloader,
    output_dir="/home/giulia/deepvalve/uncertainty_analysis/UNET-DSNT",
    device="cuda",
):
